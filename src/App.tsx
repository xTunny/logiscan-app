...
// Usamos la librería que Vercel sí reconoce en tu proyecto
...
// Mantenemos la librería de animaciones que ya tenías
...
// CONFIGURACIÓN IA CON TU API KEY REAL HARDCODEADA
...
// --- Componente ErrorBoundary para evitar pantallas blancas ---
  ...
  if (hasError) return (
    <div className="min-h-screen flex items-center justify-center bg-red-50 p-4 text-center">
      <div className="bg-white p-8 rounded-2xl shadow-xl max-w-md">
        ...
        <h2 className="text-2xl font-bold text-slate-800">Error de Sistema</h2>
        <button onClick={() => window.location.reload()} className="mt-4 bg-blue-600 text-white px-6 py-2 rounded-xl font-bold">Reiniciar Aplicación</button>
      </div>
    </div>
  );
  return <>{children}</>;
...
// --- Componentes con la ESTÉTICA ORIGINAL Recuperada ---
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        ...
        <div className="overflow-x-auto"><table className="w-full text-left border-collapse"><thead className="bg-slate-50/50 text-slate-400 text-[10px] font-bold uppercase tracking-wider"><tr><th className="px-6 py-4">Conduce #</th><th className="px-6 py-4">Suplidor</th><th className="px-6 py-4">Estado</th><th className="px-6 py-4 text-right">Acción</th></tr></thead><tbody className="divide-y divide-slate-100">
          {conduces.slice(0, 5).map(c => (
            <tr key={c.id} className="hover:bg-slate-50/50 transition-colors cursor-pointer" onClick={() => { setSelectedConduce(c); setView('audit'); }}>
              ...
              <td className="px-6 py-4"><span className={`px-2.5 py-1 rounded-full text-[10px] font-bold uppercase ${c.estado === 'completado' ? 'bg-green-100 text-green-700' : c.estado === 'pendiente_auditoria' ? 'bg-amber-100 text-amber-700' : 'bg-red-100 text-red-700'}`}>{c.estado.replace('_', ' ')}</span></td>
...
function Capture({ user, setView, setSelectedConduce }: any) {
  ...
  const processIA = async () => {
    ...
    try {
      const prompt = "Analiza este documento y extrae JSON puro: { suplidor: { nombre: '' }, registro: { fecha: '', conduce_nro: '', entregado_por: '' }, items: [ { descripcion: '', cantidad_impresa: 0, unidad: '', novedad_detectada: false } ] }";
      ...
      for (const i of data.items) { await setDoc(doc(db, 'items', `I_${Date.now()}_${Math.random().toString(36).substr(2, 5)}`), { ...i, conduce_id: cId, cantidad_recibida: null, auditado: false }); }
...
function Audit({ selectedConduce, setSelectedConduce, setView }: any) {
  ...
  if (!selectedConduce) return (
    <div className="space-y-4">
      <h2 className="text-2xl font-bold text-slate-800 mb-6 font-display">Pendientes de Auditoría</h2>
      <div className="grid grid-cols-1 gap-4">
        {pendings.map(p => (
          <div key={p.id} onClick={() => setSelectedConduce(p)} className="bg-white p-5 rounded-2xl border border-slate-200 flex justify-between items-center cursor-pointer hover:border-blue-500 shadow-sm transition-all group">
            <div className="flex items-center gap-4"><div className="bg-amber-100 p-2 rounded-lg text-amber-600 group-hover:bg-blue-100 group-hover:text-blue-600 transition-colors"><Clock className="w-5 h-5" /></div><div><p className="font-bold text-slate-800 font-display">#{p.conduce_nro}</p><p className="text-slate-500 text-sm">{p.suplidor_nombre}</p></div></div>
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      <div className="flex items-center gap-4 mb-4"><button onClick={() => setSelectedConduce(null)} className="p-2 hover:bg-slate-200 rounded-lg transition-colors"><ArrowLeft className="w-6 h-6 text-slate-600" /></button><div><h2 className="text-2xl font-bold text-slate-800 font-display">Auditando Conduce #{selectedConduce.conduce_nro}</h2><p className="text-slate-500">{selectedConduce.suplidor_nombre}</p></div></div>
      <div className="bg-white rounded-3xl border border-slate-200 shadow-xl overflow-hidden divide-y divide-slate-100">
        {items.map(i => (
          <div key={i.id} className="p-6 flex flex-col sm:flex-row justify-between sm:items-center gap-4 hover:bg-slate-50/50 transition-colors">
            <div><p className="font-bold text-slate-800 text-lg font-display">{i.descripcion}</p><p className="text-sm font-bold text-slate-400 uppercase tracking-tighter">Esperado: {i.cantidad_impresa} {i.unidad}</p></div>
        ...
        <div className="p-8 bg-slate-50"><button onClick={async () => { const hasF = items.some(i => (i.cantidad_recibida || 0) < i.cantidad_impresa); await updateDoc(doc(db, 'conduces', selectedConduce.id), { estado: hasF ? 'con_faltantes' : 'completado' }); setSelectedConduce(null); setView('dashboard'); }} className="w-full bg-green-600 text-white py-4 rounded-2xl font-bold shadow-lg hover:bg-green-700 transition-all text-lg font-display">Finalizar Auditoría</button></div>
      </div>
    </div>
  );
}
...
function Pendings({ user }: any) {
  ...
  return ( <div className="bg-white rounded-3xl border border-slate-200 overflow-hidden shadow-sm"><table className="w-full text-left border-collapse"><thead className="bg-slate-50 font-bold text-xs text-slate-400 uppercase tracking-wider"><tr><th className="p-6">Producto</th><th className="p-6">Faltante</th><th className="p-6 text-right">Suplidor</th></tr></thead><tbody className="divide-y divide-slate-100">{p.map(x => (<tr key={x.id} className="hover:bg-slate-50/50 transition-colors"><td className="p-6 font-bold text-slate-800 font-display">{x.descripcion}</td><td className="p-6 text-red-600 font-black">{x.cantidad_faltante}</td><td className="p-6 text-slate-500 text-right">{x.suplidor_nombre}</td></tr>))}</tbody></table></div> );
}

function History({ conduces }: any) {
  return ( <div className="max-w-5xl mx-auto space-y-4 font-display">{conduces.map((c: any) => ( <div key={c.id} className="bg-white p-5 rounded-2xl border border-slate-200 flex justify-between items-center shadow-sm hover:shadow-md transition-all group"><div className="flex items-center gap-4"><div className="bg-slate-50 p-2 rounded-lg text-slate-400 group-hover:bg-blue-50 group-hover:text-blue-600 transition-colors"><FileText className="w-6 h-6" /></div><div><p className="font-bold text-slate-800 text-lg">#{c.conduce_nro}</p><p className="text-slate-500 text-sm font-bold uppercase tracking-tighter">{c.suplidor_nombre}</p></div></div><span className={`text-[10px] px-4 py-1.5 rounded-full font-black uppercase tracking-widest shadow-sm ${c.estado === 'completado' ? 'bg-green-100 text-green-700' : 'bg-blue-100 text-blue-700'}`}>{c.estado.replace('_',' ')}</span></div> ))}</div> );
}

function Reports({ conduces }: any) {
  const exportar = () => { const ws = XLSX.utils.json_to_sheet(conduces); const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, ws, "Inventario"); XLSX.writeFile(wb, "Reporte_Completo_LogiScanAI.xlsx"); };
  return ( <div className="bg-white p-16 rounded-[40px] border border-slate-200 text-center shadow-sm max-w-2xl mx-auto mt-10 shadow-blue-50/50"><div className="w-24 h-24 bg-blue-50 rounded-3xl flex items-center justify-center mx-auto mb-8 text-blue-600 shadow-inner"><BarChart3 className="w-12 h-12" /></div><h2 className="text-3xl font-black text-slate-800 mb-4 font-display">Exportar Operaciones</h2><p className="text-slate-500 mb-10 text-lg">Descarga el historial completo en formato Excel para auditorías externas.</p><button onClick={exportar} className="bg-blue-600 text-white px-12 py-5 rounded-[20px] font-black text-xl flex gap-4 mx-auto items-center hover:bg-blue-700 transition-all shadow-xl shadow-blue-100 active:scale-95"><Download className="w-7 h-7" /> Generar Excel</button></div> );
}
