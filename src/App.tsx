...
// Cambiado a framer-motion para que Vercel no de error de compilación
...
// CONFIGURACIÓN CON TU LLAVE DE IA
  ...
  if (!user) return (
    <div className="min-h-screen bg-blue-600 flex items-center justify-center p-4">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white p-8 rounded-3xl shadow-2xl text-center max-w-md w-full">
        ...
        <button onClick={() => signInWithPopup(auth, googleProvider)} className="w-full bg-blue-600 text-white py-4 rounded-2xl font-bold flex items-center justify-center gap-2 hover:bg-blue-700">
      ...
      <div className="min-h-screen bg-slate-50 flex">
        ...
        <main className="flex-1 p-6 lg:p-10 overflow-auto">
          <AnimatePresence mode="wait">
            {view === 'dashboard' && <Dashboard stats={{total: conduces.length, pendientes: conduces.filter(c=>c.estado==='pendiente_auditoria').length}} setView={setView} />}
...
function Dashboard({ stats, setView }: any) {
  return (
    <div className="space-y-8">
      <div className="flex justify-between items-center"><h2 className="text-2xl font-bold">Dashboard</h2><button onClick={() => setView('capture')} className="bg-blue-600 text-white px-6 py-3 rounded-xl font-bold flex gap-2"><Plus /> Nueva Captura</button></div>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-white p-6 rounded-2xl border shadow-sm"><p className="text-slate-400 text-xs font-bold uppercase">Total Conduces</p><p className="text-3xl font-black">{stats.total}</p></div>
        <div className="bg-white p-6 rounded-2xl border shadow-sm"><p className="text-slate-400 text-xs font-bold uppercase">Pendientes</p><p className="text-3xl font-black text-amber-500">{stats.pendientes}</p></div>
      </div>
    </div>
  );
}
    ...
    <div className="max-w-xl mx-auto bg-white p-8 rounded-3xl shadow-xl border">
      {!image ? (
        ...
        <div className="space-y-6">
          <img src={image} className="w-full rounded-2xl shadow-inner" alt="Preview" />
          {loading ? <div className="text-center font-bold text-blue-600"><Loader2 className="animate-spin mx-auto mb-2 w-8 h-8" /> Leyendo con IA...</div> : <button onClick={processIA} className="w-full bg-blue-600 text-white py-4 rounded-2xl font-bold shadow-lg">Procesar Información</button>}
          <button onClick={()=>setImage(null)} className="w-full text-slate-400 text-sm font-medium">Repetir foto</button>
        </div>
      )}
    </div>
  );
...
function Audit({ selectedConduce, setSelectedConduce, setView }: any) {
  ...
  if (!selectedConduce) return (
    <div className="space-y-4">
      <h2 className="text-2xl font-bold mb-6">Pendientes de Auditoría</h2>
      {pendings.map(p => ( <div key={p.id} onClick={() => setSelectedConduce(p)} className="bg-white p-5 rounded-2xl border flex justify-between items-center cursor-pointer hover:border-blue-500 shadow-sm transition-all"><p className="font-bold">#{p.conduce_nro}</p><p className="text-slate-500 text-sm">{p.suplidor_nombre}</p><ChevronRight className="text-slate-300" /></div> ))}
    </div>
  );

  return (
    <div className="bg-white p-8 rounded-3xl border shadow-lg space-y-6">
      <h2 className="text-2xl font-bold">Auditando #{selectedConduce.conduce_nro}</h2>
      <div className="divide-y">
        {items.map(i => (
          <div key={i.id} className="py-4 flex justify-between items-center">
            <div><p className="font-bold text-slate-800">{i.descripcion}</p><p className="text-xs text-slate-400">Solicitado: {i.cantidad_impresa} {i.unidad}</p></div>
            <input type="number" className="w-24 border rounded-xl p-3 text-center font-bold" placeholder="Cant" onChange={(e) => updateDoc(doc(db, 'items', i.id), { cantidad_recibida: Number(e.target.value), auditado: true })} />
          </div>
        ))}
      </div>
      <button onClick={() => { setSelectedConduce(null); setView('dashboard'); }} className="w-full bg-green-600 text-white py-4 rounded-2xl font-bold shadow-lg">Finalizar Auditoría</button>
    </div>
  );
}
...
function Pendings({ user }: any) {
  ...
  return ( <div className="bg-white rounded-3xl border overflow-hidden shadow-sm"><table className="w-full text-left"><thead className="bg-slate-50 font-bold"><tr><th className="p-5">Producto</th><th className="p-5">Faltante</th></tr></thead><tbody>{p.map(x => (<tr key={x.id} className="border-t"><td className="p-5 font-bold text-slate-800">{x.descripcion}</td><td className="p-5 text-red-600 font-black">{x.cantidad_faltante}</td></tr>))}</tbody></table></div> );
}

function History({ conduces }: any) {
  return ( <div className="space-y-3">{conduces.map((c: any) => ( <div key={c.id} className="bg-white p-5 rounded-2xl border flex justify-between items-center shadow-sm"><p className="font-bold">#{c.conduce_nro}</p><span className="text-[10px] bg-slate-100 px-3 py-1 rounded-full font-bold text-slate-600 uppercase tracking-tighter">{c.estado.replace('_',' ')}</span></div> ))}</div> );
}

function Reports({ conduces }: any) {
  const exportExcel = () => { const ws = XLSX.utils.json_to_sheet(conduces); const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, ws, "Data"); XLSX.writeFile(wb, "Reporte_LogiScan.xlsx"); };
  return ( <div className="bg-white p-12 rounded-3xl border text-center shadow-sm max-w-lg mx-auto"><BarChart3 className="w-16 h-16 text-blue-600 mx-auto mb-6" /><button onClick={exportExcel} className="bg-blue-600 text-white px-10 py-4 rounded-2xl font-bold flex gap-3 mx-auto items-center"><Download /> Descargar Reporte Excel</button></div> );
}
